...
impl Database {
    ...
    pub fn open<P: AsRef<Path>>(file: P) -> Result<Self, DatabaseError> {
        let mut file = File::open(file).map_err(|e| DatabaseError::FileSystemError(format!("{e:?}")))?;
        let metadata = file.metadata().map_err(|e| DatabaseError::FileSystemError(format!("{e:?}")))?;
        let mut data = vec![0; metadata.len() as usize];
        file.read_exact(&mut data).map_err(|e| DatabaseError::FileSystemError(format!("{e:?}")))?;
        Database::load(data)
    }
    ...
    pub fn print_info(&self) {
        ...
        println!("{:<20} {}", "file change counter:", header.file_change_counter);
        println!("{:<20} {}", "database page count:", header.database_page_count);
        println!("{:<20} {}", "freelist page count:", header.freelist_page_count);
        println!("{:<20} {}", "schema cookie:", header.schema_cookie);
        println!("{:<20} {}", "schema format:", header.schema_format as u8);
        println!("{:<20} {}", "default cache size:", header.default_page_cache_size);
        println!("{:<20} {}", "autovacuum top root:", header.autovacuum_top_root);
        println!("{:<20} {}", "incremental vacuum:", header.incremental_vacuum_mode as u8);
